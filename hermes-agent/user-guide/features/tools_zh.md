...
工具是扩展 Agent 功能的函数。它们被组织成逻辑**工具集**，可按平台启用或禁用。

## 可用工具

Hermes 配备了广泛的内置工具注册表，涵盖网页搜索、浏览器自动化、终端执行、文件编辑、记忆、委托、RL 训练、消息发送、Home Assistant 等。

:::note
**Honcho 跨会话内存**作为内存提供商插件提供（`plugins/memory/honcho/`），而非内置工具集。参见 [Plugins](./plugins.md) 了解安装方法。
:::
...
| **自动化和发送** | `cronjob`, `send_message` | 定时任务（创建/列表/更新/暂停/恢复/运行/删除操作）和出站消息发送。 |
| **集成** | `ha_*`, MCP 服务器工具, `rl_*` | Home Assistant、MCP、RL 训练和其他集成。 |

有关权威的代码派生注册表，请参见 [Built-in Tools Reference](/docs/reference/tools-reference) 和 [Toolsets Reference](/docs/reference/toolsets-reference)。
...
参见 [Toolsets Reference](/docs/reference/toolsets-reference) 以获取完整集合，包括平台预设如 `hermes-cli`、`hermes-telegram` 和动态 MCP 工具集如 `mcp-<server>`。
...
| 后端 | 描述 | 用途 |
|---------|-------------|----------|
| `local` | 在你的计算机上运行（默认） | 开发、可信任务 |
| `docker` | 隔离的容器 | 安全、可重复性 |
| `ssh` | 远程服务器 | 沙箱、让 Agent 远离自己的代码 |
...
Docker 可选择性地通过 `terminal.docker_forward_env` 接收显式环境允许列表，但转发的变量对容器内命令可见，应视为暴露给该会话。
...
如果命令需要 sudo，你会被提示输入密码（为该会话缓存）。或在 `~/.hermes/.env` 中设置 `SUDO_PASSWORD`。
